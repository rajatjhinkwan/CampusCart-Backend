...
const { searchTrie } = require("../utils/dsaManager");
...
const getSearchSuggestions = handleAsync(async (req, res) => {
  ...
  // Use Trie for O(L) search time instead of O(N) database scan
  // This makes autocomplete extremely fast and reliable
  const suggestions = searchTrie.startsWith(query.trim());
  
  // Format results to return just the text or limited objects
  const results = suggestions.slice(0, 10).map(s => s.word);
